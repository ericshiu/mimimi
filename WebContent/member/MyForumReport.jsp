...
<%
	session.setAttribute("pageReq", "/member/personal/MyForumReport.jsp");
%>
<%
			...
			<div class="col-xs-12 col-lg-12 right1"
				...
				<div id="exTab2" class="container" style="width: 100%;">
					<ul class="nav nav-tabs">
						...
						<li id="li1"><a href="#1" data-toggle="tab">被檢舉</a></li>
								...
								<table class="table table-hover" style="text-align: center;">
									<thead>
										<tr>
											...
											<th style="width: 20%">檢舉文章</th>
											<th style="width: 20%">檢舉標題</th>
								...
								<div style="overflow: auto; height: 600px;">
									<table class="table table-hover" style="text-align: center;">
										...
										<tbody>
											...
											<%int q=0; %>
											<c:forEach var="FRVO" items="${FRSvc.desc}">
												<c:if test="${FRVO.mem_no==userVO.mem_no }">
													<tr><%q++; %>
														<td style="width: 10%"><%=q %></td>
														<td style="width: 20%">${FRVO.fr_no}</td>
														<td style="width: 20%">${FRVO.fr_am_no}</td>
														<th style="width: 20%">${FRVO.fr_title}</th>
														<td style="width: 20%"><fmt:formatDate value="${FRVO.fr_publish_date}" pattern="yyyy-MM-dd aaa HH:mm:ss "/></td>
														
														<c:if test="${FRVO.fr_status=='0' }">
														<td style="width: 10%">未審核</td>
														</c:if>
														<c:if test="${FRVO.fr_status=='1' }">
									...
									<table class="table table-hover" style="text-align: center;">
										...
										<tbody>
											...
											<c:forEach var="FRVO" items="${FRSvc.desc}">
												...
												<c:if test="${not empty FASvc.getOneFa(FRVO.fr_am_no)}">
												<c:if test="${FASvc.getOneFa(FRVO.fr_am_no).mem_no==userVO.mem_no}">
													<tr><%w++; %>
														<td style="width: 10%"><%=w %></td>
														<td style="width: 20%">${FRVO.fr_no}</td>
														<td style="width: 20%">${FRVO.fr_am_no}</td>
														<th style="width: 20%">${FRVO.fr_title}</th>
														<td style="width: 20%">${FRVO.fr_publish_date}</td>
														<c:if test="${FRVO.fr_status=='0' }">
														<td style="width: 10%">未審核</td>
														</c:if>
														<c:if test="${FRVO.fr_status=='1' }">
