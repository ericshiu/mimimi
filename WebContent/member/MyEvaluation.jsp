...
<%
	session.setAttribute("pageReq", "/member/personal/MyEvaluation.jsp");
%>
<%
	...
	<div class="container"
		...
		<div class="col-xs-12  col-lg-10 right"
			...
			<div class="col-xs-12 col-lg-12 right1"
				style="background-color: #background-color:fff; padding-left: 10px; width: 100%;">


				<div id="exTab2" class="container" style="width: 100%;">
					<ul class="nav nav-tabs">
						<li id="li0" class="active"><a href="#0" data-toggle="tab">產後護理</a></li>
						<li id="li1"><a href="#1" data-toggle="tab">商場</a></li>
						...
						<div class="tab-pane active" id="0">
							...
							<div class="panel panel-success">
								<div class="panel-heading">
									<h3 class="panel-title" style="text-align: left;">評價紀錄</h3>
								</div>
									...
									<table class="table table-hover" style="text-align: center;">
										...
										<tbody>
											<jsp:useBean id="PCSvc" scope="page"
												class="com.postpartum_care.model.PostpartumCareService" />
											<jsp:useBean id="FESvc" scope="page"
												class="com.firm_evaluation.model.FirmEvaluationService" />
											<%
												int q = 0;
											%>
											<c:forEach var="FEVO"
												items="${FESvc.getAllByEvaMem_no(userVO.mem_no)}">
												<c:if test="${!empty PCSvc.getOnePC(FEVO.fp_no)}">
													<tr>
														<%
															q++;
														%>
														<td style="width: 10%"><%=q%></td>
														<td style="width: 20%">${FEVO.fe_no}</td>
														<td style="width: 20%">${PCSvc.getOnePC(FEVO.fp_no).pc_name}</td>
														<th style="width: 10%">${FEVO.fe_type}</th>
														<td style="width: 20%">${FEVO.fe_content}</td>
														<td style="width: 20%"><fmt:formatDate
																value="${FEVO.fe_date}"
																pattern="yyyy-MM-dd aaa HH:mm:ss " /></td>
								...
								<div style="overflow: auto; height: 600px;">
									<table class="table table-hover" style="text-align: center;">
										...
										<tbody>
											...
											<jsp:useBean id="FirSvc" scope="page"
											...
											<c:forEach var="FEVO"
												items="${FESvc.getAllByEvaMem_no(userVO.mem_no)}">
												<c:if test="${!empty FirSvc.getOneFir(FEVO.fp_no)}">
													<tr>
														<%
															w++;
														%>
														...
														<td style="width: 20%">${FEVO.fe_no}</td>
														<td style="width: 20%">${FirSvc.getOneFir(FEVO.fp_no).fir_name}</td>
														<th style="width: 10%">${FEVO.fe_type}</th>
														<td style="width: 20%">${FEVO.fe_content}</td>
														<td style="width: 20%"><fmt:formatDate
																value="${FEVO.fe_date}"
