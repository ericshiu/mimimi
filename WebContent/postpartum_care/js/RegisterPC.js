...
function babyHelper(){
//	$(pc_id).val("mommylovebaby");
	$('#pc_name').val("媽咪樂寶月子中心-二號店");
	  $('#pc_type1').prop("checked", true);
	  $('#pc_type1_label').addClass("active");
	  $('#pc_type1_label').addClass("focus");
	  $('#pc_phone').val("03 456 8888");
	  $('#pc_email').val("1u4c93x06@gmail.com");
	  $('#city_select').empty();
	  $('#city_select').append("<option value=''>桃園市</option>");
	  $('#area_select').empty();
	  $('#area_select').append("<option value=''>中壢區</option>");
	  $('#resault').val("320桃園市中壢區中大路183號");
	  $('#resault2').val("320桃園市中壢區中大路183號");
	  $('#pc_point').val(50);
	  $('#pc_gift1').prop("checked", true);
	  $('#pc_gift1_label').addClass("active");
	  $('#pc_gift1_label').addClass("focus");
	  $('#pc_bonus1').prop("checked", true);
	  $('#pc_bonus1_label').addClass("active");
	  $('#pc_bonus1_label').addClass("focus");
}
...
//		alert(parent.lastChild.childNodes.length);
...
//照片說明區	
	...
	span1.setAttribute("class","input-group-addon input-sum");
	span1.innerText="照片說明";
	div2.appendChild(span1);
	var input1 = document.createElement('input');
	input1.type="text";
	input1.setAttribute("class","form-control");
	input1.setAttribute("name","pcp_summary");
	input1.setAttribute("value",$id("pc_name").value);
...
//照片新增區	
	...
	input3.setAttribute("name","pcp_picture");	
